...
function fmtGuide(txt){
  ...
  return txt.split(String.fromCharCode(10)).map(function(line){
    ...
    if(t.startsWith('•')){
      return '<div style="display:flex;gap:7px;margin:.2rem 0"><span style="color:var(--g4);flex-shrink:0;margin-top:.05rem">•</span><span style="font-size:.77rem;color:var(--i2);line-height:1.55">'+lk(t.slice(1).trim())+'</span></div>';
    }
    if(t.startsWith('→')){
      return '<div style="display:flex;gap:7px;margin:.3rem 0;background:var(--g8);border-radius:6px;padding:.45rem .6rem"><span style="color:var(--g3);flex-shrink:0">→</span><span style="font-size:.77rem;color:var(--g2);line-height:1.55;font-weight:600">'+lk(t.slice(1).trim())+'</span></div>';
    }
    if(/^[0-9]+\./.test(t)){
      var num=t.match(/^([0-9]+)\.(.*)$/);
    ...
    return '<p style="font-size:.77rem;color:var(--i2);line-height:1.6;margin:.2rem 0">'+lk(t)+'</p>';
  }).join('');
}

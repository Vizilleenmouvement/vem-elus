...
const ProjetJalons = {
  ...
  insert(pid, d) { const r=db.prepare('INSERT INTO projet_jalons (projet_id,titre,date_jalon,statut,notes) VALUES (?,?,?,?,?)').run(pid,d.titre||'',d.date||d.date_jalon||'',d.statut||'prevu',d.notes||''); return db.prepare('SELECT * FROM projet_jalons WHERE id=?').get(r.lastInsertRowid); },
  patch(id, d) { const j=db.prepare('SELECT * FROM projet_jalons WHERE id=?').get(id); if(!j) return false; db.prepare('UPDATE projet_jalons SET titre=?,date_jalon=?,statut=?,notes=? WHERE id=?').run(d.titre||j.titre,d.date||j.date_jalon||'',d.statut||j.statut,d.notes!==undefined?d.notes:j.notes,id); return true; },
...
const ProjetPartenaires = {
  ...
  insert(pid, d) { const r=db.prepare('INSERT INTO projet_partenaires (projet_id,nom,type,contact,email,tel) VALUES (?,?,?,?,?,?)').run(pid,d.nom||'',d.type||'',d.contact||'',d.email||'',d.tel||''); return db.prepare('SELECT * FROM projet_partenaires WHERE id=?').get(r.lastInsertRowid); },
...
const ProjetDocs = {
  ...
  insert(pid, d) { const r=db.prepare('INSERT INTO projet_docs (projet_id,titre,type,url,description) VALUES (?,?,?,?,?)').run(pid,d.titre||'',d.type||'Autre',d.url||'',d.description||''); return db.prepare('SELECT * FROM projet_docs WHERE id=?').get(r.lastInsertRowid); },
...
const ProjetPresse = {
  ...
  insert(pid, d) { const r=db.prepare('INSERT INTO projet_presse (projet_id,titre,media,url,date_pub) VALUES (?,?,?,?,?)').run(pid,d.titre||'',d.media||'',d.url||'',d.date||d.date_pub||''); return db.prepare('SELECT * FROM projet_presse WHERE id=?').get(r.lastInsertRowid); },

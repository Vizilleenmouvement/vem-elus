...
const ProjetJalons = {
  get(pid) { return db.prepare('SELECT * FROM projet_jalons WHERE projet_id=? ORDER BY date').all(pid); },
  insert(pid,d) { const r=db.prepare('INSERT INTO projet_jalons (projet_id,titre,date,statut,notes) VALUES (?,?,?,?,?)').run(pid,d.titre||'',d.date||'',d.statut||'prévu',d.notes||''); return db.prepare('SELECT * FROM projet_jalons WHERE id=?').get(r.lastInsertRowid); },
  update(id,d) { db.prepare('UPDATE projet_jalons SET titre=?,date=?,statut=?,notes=? WHERE id=?').run(d.titre,d.date,d.statut||'prévu',d.notes||'',id); },
  delete(id) { db.prepare('DELETE FROM projet_jalons WHERE id=?').run(id); }
};

const ProjetPartenaires = {
  get(pid) { return db.prepare('SELECT * FROM projet_partenaires WHERE projet_id=? ORDER BY nom').all(pid); },
  insert(pid,d) { const r=db.prepare('INSERT INTO projet_partenaires (projet_id,nom,type,contact,email,tel,notes) VALUES (?,?,?,?,?,?,?)').run(pid,d.nom||'',d.type||'institutionnel',d.contact||'',d.email||'',d.tel||'',d.notes||''); return db.prepare('SELECT * FROM projet_partenaires WHERE id=?').get(r.lastInsertRowid); },
  delete(id) { db.prepare('DELETE FROM projet_partenaires WHERE id=?').run(id); }
};

const ProjetContacts = {
  get(pid) { return db.prepare('SELECT * FROM projet_contacts WHERE projet_id=? ORDER BY nom').all(pid); },
...
const ProjetDocs = {
  ...
  insert(pid,d) { const r=db.prepare('INSERT INTO projet_docs (projet_id,titre,url,type,description) VALUES (?,?,?,?,?)').run(pid,d.titre||'',d.url||'',d.type||'document',d.description||''); return db.prepare('SELECT * FROM projet_docs WHERE id=?').get(r.lastInsertRowid); },
  delete(id) { db.prepare('DELETE FROM projet_docs WHERE id=?').run(id); }
};

const ProjetPresse = {
  get(pid) { return db.prepare('SELECT * FROM projet_presse WHERE projet_id=? ORDER BY date_pub DESC').all(pid); },
  insert(pid,d) { const r=db.prepare('INSERT INTO projet_presse (projet_id,titre,url,media,date_pub,type) VALUES (?,?,?,?,?,?)').run(pid,d.titre||'',d.url||'',d.media||'',d.date_pub||'',d.type||'article'); return db.prepare('SELECT * FROM projet_presse WHERE id=?').get(r.lastInsertRowid); },
...
module.exports = {db, Elus, Agenda, Projets, Statuts, CR, Biblio, Signalements, Evenements, Chat, Annonces, Tasks, Notifs, RepElus, stats:dbStats, ts, nid, ProjetJalons, ProjetPartenaires, ProjetContacts, ProjetDocs, ProjetPresse, ProjetJournal};
